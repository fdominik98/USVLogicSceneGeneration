...
class DeapAlgorithm(GeneticAlgorithmBase):
    ...
    def init_problem(self, initial_population : list[list[float]], eval_data : EvaluationData) -> None:
        ...
        def generate_individual(actors):
            return [np.random.uniform(low, high) for low, high in boundaries] * actors
        ...
        toolbox.register("mutate", tools.mutPolynomialBounded, low=[b[0] for b in boundaries]* self.env_config.actor_num,
                        up=[b[1] for b in boundaries] * self.env_config.actor_num, eta=eval_data.mutate_eta, indpb=1/self.env_config.variable_num)
        ...
        halloffame = tools.HallOfFame(1)  # Keep track of the best individuals
