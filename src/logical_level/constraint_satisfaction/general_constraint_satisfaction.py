...
class GeneralConstraintSatisfaction():
    def __init__(self, solver : Solver, measurement_name: str, scenarios: List[Scenario], test_config : EvaluationData,
                 number_of_runs : int, warmups : int, verbose: bool) -> None:
        self.solver = solver
        self.measurement_name = measurement_name
        self.scenarios : List[Tuple[LogicalScenario, Optional[FunctionalScenario]]] = [LogicalScenarioBuilder.build(scenario, test_config.init_method) for scenario in scenarios]
    ...
    def run(self, core_id : int):
        ...
        for logical_scenario, functional_scenario in islice(cycle(self.scenarios), self.warmups):
            self.__evaluate(logical_scenario, functional_scenario, core_id, False)
        
        for logical_scenario, functional_scenario in islice(cycle(self.scenarios), self.number_of_runs):
            self.__evaluate(logical_scenario, functional_scenario, core_id, True)
         
    def __evaluate(self, logical_scenario : LogicalScenario, functional_scenario : Optional[FunctionalScenario],
                   core_id : int,  save : bool):
        try:
