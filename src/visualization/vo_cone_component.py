...
class VOConeComponent(PlotComponent):
    ...
    def do_draw(self, zorder : int):
        for colreg_s in self.env.colreg_situations:
            ...
            cone1 = o2.v + o1.p + np.array([np.cos(angle1), np.sin(angle1)]) * colreg_s.o_distance / 2.5
            cone2 = o2.v + o1.p + np.array([np.cos(angle2), np.sin(angle2)]) * colreg_s.o_distance / 2.5
            ...
            other_velocity = self.ax.quiver(o1.p[0], o1.p[1], o2.v[0], o2.v[1], angles='xy', scale_units='xy', scale=1, color=colors[o2.id], zorder=zorder)
