...
class ShipMarkingsComponent(PlotComponent):
    ...
    def do_draw(self, zorder : int):
        for o in self.env.vessels:
            ...
            #radius_circle = plt.Circle(o.p, o.l, color=colors[o.id], fill=False, linestyle='--', label=f'{name} Radius: {o.l}m', zorder=zorder)
            ...
            ship_vel = self.ax.quiver(o.p[0], o.p[1], o.v[0], o.v[1], angles='xy', scale_units='xy', scale=1, color=colors[o.id], label=f'{name} Velocity: {angle}, {speed}', zorder=zorder-1)

            # Plot the positions
            ship_dot = self.ax.scatter(o.p[0], o.p[1], color=colors[o.id], s=100, label=f'{name} Position: ({o.p[0]:.2f}, {o.p[1]:.2f})', zorder=zorder)
