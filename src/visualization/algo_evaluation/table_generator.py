...
class TableGenerator():
    ...
    def generate_stat_sign_table(self,):
        ...
        latex_code += "    n & {} & {} & {} & {} \\\\\n".format(*[3, 4, 5, 6])
        latex_code += "    \\midrule\n"
        latex_code += "    \\textit{{p}} rt. & {:.1e} & {:.1e} & {:.1e} & {:.1e} \\\\\n".format(*[list(st.p_values.values())[0] for st in self.stat_sign_runtime])
        latex_code += "    \\textit{{e}} rt. & {:.1f} & {:.1f} & {:.1f} & {:.1f} \\\\\n".format(*[list(st.effect_sizes.values())[0][0] for st in self.stat_sign_runtime])
        latex_code += "    \\textit{{p}} suc. r. & {:.1e} & {:.1e} & {:.1e} & {:.1e} \\\\\n".format(*[list(st.p_values.values())[0] for st in self.stat_sign_success])
        latex_code += "    \\textit{{e}} suc. r. & {:.1f} & {:.1f} & {:.1f} & {:.1f} \\\\\n".format(*[list(st.odds_ratios.values())[0] for st in self.stat_sign_success])
