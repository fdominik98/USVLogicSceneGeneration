...
from evaluation.chi_square_cramers_v import ChiSquareCramersV
...
class ScenarioTypeStatisticsTable(DummyEvalPlot):  
    ...
    def create_fig(self) -> plt.Figure:
        samples : Dict[int, Dict[str, Tuple[List[int], List[int]]]] = defaultdict(lambda: defaultdict())
        for i, vessel_number in enumerate(self.vessel_numbers):
            for j, comparison_group in enumerate(self.comparison_groups):
                ...
                samples[vessel_number][comparison_group] = (values, [0] * values[0] + [1] * values[1] + [2] * values[2])
                
            values = [round(value) for value in [56952*0.131, 56952*0.002, 56952*0.867]]
            samples[vessel_number]['zhu_et_al'] = (values, [0] * values[0] + [1] * values[1] + [2] * values[2])
            values = [round(value) for value in VesselTypeSampler.sample(FunctionalModelManager.get_x_vessel_scenarios(vessel_number), 0, {})]
            samples[vessel_number]['base_reference'] = (values, [0] * values[0] + [1] * values[1] + [2] * values[2])
        ...
        for i, vessel_number in enumerate(self.vessel_numbers):
            for j, (group1, group2) in enumerate(groups_to_compare):
                ...
                test = ChiSquareCramersV(*samples1, *samples2)
                print(f'{vessel_number} vessels, {group1} - {group2}: p-value:{test.p_value}, KL Divergence::{test.kl_div}')
