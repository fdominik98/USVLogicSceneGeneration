            ...
            for j, config_group in enumerate(self.comparison_groups):
                ...
                for eval_data in self.measurements[actor_number_by_type][config_group]:
                    if eval_data.is_valid:
                        ...
                        relevant_values.append(sum(1 for scenario, count in equivalence_classes.values() if scenario.functional_scenario.is_relevant))
                        ambiguous_values.append(sum(1 for scenario, count in equivalence_classes.values() if scenario.functional_scenario.is_ambiguous))
                    else:
